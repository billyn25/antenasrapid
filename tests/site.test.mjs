...
test('ocho páginas separadas sin importar otras localidades',()=>{assert.equal(pages.length,8);assert.equal(pages.filter(p=>p.type==='province').length,5);assert.equal(pages.filter(p=>p.type==='town').length,2);});
...
test('crea HTML real, 404 y cabeceras sin sitemap incompleto',()=>{const dir=fs.mkdtempSync(path.join(os.tmpdir(),'rapid-test-'));try{assert.equal(build(dir),8);assert.ok(fs.existsSync(path.join(dir,'404.html')));assert.match(fs.readFileSync(path.join(dir,'_headers'),'utf8'),/noindex/);assert.ok(!fs.existsSync(path.join(dir,'sitemap.xml')));}finally{fs.rmSync(dir,{recursive:true,force:true});}});
